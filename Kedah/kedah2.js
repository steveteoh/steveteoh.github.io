...
var stateRequestURL = 'https://steveteoh.github.io/Maps/geoBoundaries-MYS-ADM1_simplified.geojson';
var districtRequestURL = 'https://steveteoh.github.io/Maps/states/kedah.geojson';
var mapID = "Kedah";
var inputURL = 'https://steveteoh.github.io/data/kedah/kedah.csv';
//var inputURL = 'http://localhost:1337/data/kedah.csv';
...
$(window).load(function () {
    ...
    //determine if the inputURL exists
    while (!UrlExists(inputURL)) {
        //add in the date prefix for previous dates
        var underscore = inputURL.includes('_');   //determine if there is an underscore (_) in the filename
        var datestr = 0;
        var website = inputURL.slice(0, inputURL.lastIndexOf('/') + 1);
        var filename = inputURL.slice(inputURL.lastIndexOf('/') + 1);

        console.log("before _=" + underscore + ", datestr =" + datestr + ", filename=" + filename + ", inputurl=" + inputURL);

        if (underscore == false) {
            //supplied filename (not found) is just whatever.csv without a date prefix, so we add one to it.
            datestr = parseInt(new Date().toISOString().split('T')[0].replaceAll('-', ''));  //today's date
            inputURL = website + datestr + "_" + filename;  //if file does not exist, modify the url to include the datestr
        }
        else {
            //supplied filename (not found) is date_whatever.csv
            //if datestr exists, take all the text between last '/' and '_'. Since date cannot be found, we take the previous
            datestr = parseInt(inputURL.slice(inputURL.lastIndexOf('/') + 1, inputURL.indexOf('_'))) - 1; //decrement 1
            filename = inputURL.slice(inputURL.lastIndexOf('_') + 1);
            inputURL = website + datestr + "_" + filename;  //if file does not exist, modify the url to include the datestr
        }
        console.log("after _=" + underscore + ", datestr =" + datestr + ", filename=" + filename + ", inputurl=" + inputURL);
        //    inputURL = datestr + filename;  //if file does not exist, modify the url to include the datestr
    };

    //one for all
    drawDistrict(districtRequestURL, inputURL, mapID);
...
function drawDistrict(districtRequestURL, inputURL, mapID) {
    ...
    mylayer.loadGeoJson(districtRequestURL, { idPropertyName: 'Nama' },
        function (features) {
            ...
            document.getElementById('LastUpdate').innerText += mapID + ` updated: ${new Date(data[0]['timestamp']).toLocaleString('en-GB', { timeZone: 'Asia/Kuala_Lumpur' })} (${subtotal.toString()} cases),    `;
